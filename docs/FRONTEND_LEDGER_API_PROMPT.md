# 가계부 공동 사용(초대코드) API — 프론트엔드 연동 작업 프롬프트

아래 내용을 **프론트엔드 에이전트/개발자**에게 전달해 API 연동을 요청하세요.

---

## 배경

- 백엔드에 **가계부 공동 사용** 기능이 추가되었습니다.
- 가계부마다 **6자리 숫자 초대코드**가 있고, **한 가계부에 최대 2명**만 참여할 수 있습니다.
- **설정 탭**에 다음이 필요합니다:
  1. **가계부로 초대하기**: 6자리 초대코드 생성/표시
  2. **가계부에 참여하기**: 초대코드 입력으로 다른 가계부 참여
  3. **가계부 설정**: 가계부 번호 선택 후 가계부 이름 등 설정
  4. **가계부 삭제**: 가계부 코드(6자리) 입력으로 해당 가계부 삭제 (멤버만 가능)

- **entries(내역)**와 **summary(요약)**는 이제 **가계부(ledgerId)** 단위로 동작합니다.  
  사용자가 속한 가계부 목록을 불러와, 선택한 가계부의 `ledgerId`로 내역/요약 API를 호출해야 합니다.

---

## API 요약 (모두 인증 필요: `Authorization: Bearer <JWT>`)

| 용도 | 메서드 | 경로 | 요약 |
|------|--------|------|------|
| 내 가계부 목록 | GET | `/api/v1/ledgers` | 내가 속한 가계부 목록 (ledgerId, name) |
| 초대코드 생성 | POST | `/api/v1/ledgers/invite-code` | 새 가계부 생성 후 6자리 코드 반환 |
| 가계부 참여 | POST | `/api/v1/ledgers/join` | Body: `{ "code": "123456" }` |
| 가계부 설정 수정 | PATCH | `/api/v1/ledgers/:ledgerId` | Body: `{ "name": "이름" }` (멤버만) |
| 가계부 삭제 | DELETE | `/api/v1/ledgers/:ledgerId` | 6자리 코드로 삭제 (멤버만) |
| 내역 목록 | GET | `/api/v1/entries?ledgerId=123456` | 해당 가계부 내역 |
| 내역 추가 | POST | `/api/v1/entries` | Body에 `ledgerId`(6자리) 포함 |
| 내역 수정 | PUT | `/api/v1/entries/:id` | Body에 `ledgerId` 포함 권장 |
| 내역 삭제 | DELETE | `/api/v1/entries/:id` | - |
| 내역 일괄 import | POST | `/api/v1/entries/import` | Body: `{ "ledgerId": "123456", "entries": [...] }` |
| 요약 | GET | `/api/v1/summary?ledgerId=123456&period=YYYY-MM` | 해당 가계부·해당 월 요약 |

---

## 상세 스펙

### 1. GET `/api/v1/ledgers`

- **응답 예시**
  ```json
  {
    "ledgers": [
      { "ledgerId": "123456", "name": "우리집 가계부" },
      { "ledgerId": "789012", "name": null }
    ]
  }
  ```
- `name`이 없으면 `null`. 표시 시 `name || ledgerId` (예: "우리집 가계부" 또는 "123456")로 쓰면 됩니다.
- 앱 진입 시 또는 설정 화면 진입 시 호출해, **현재 사용자가 속한 가계부 목록**을 받습니다.  
  이 목록으로 “현재 선택된 가계부”를 정하고, entries/summary 호출 시 해당 `ledgerId`를 사용합니다.

### 2. POST `/api/v1/ledgers/invite-code`

- **Body**: 없음
- **응답 예시**
  ```json
  { "ledgerId": "123456", "code": "123456" }
  ```
- **설정 > 가계부로 초대하기** 버튼 클릭 시 호출합니다.  
  새 가계부가 생성되고, 현재 사용자가 멤버로 추가되며, 6자리 초대코드가 반환됩니다.  
  이 코드를 다른 사람에게 알려주면, 그 사람이 “가계부에 참여하기”에서 입력해 들어올 수 있습니다.

### 3. POST `/api/v1/ledgers/join`

- **Body**
  ```json
  { "code": "123456" }
  ```
- **성공 시**
  ```json
  { "ledgerId": "123456" }
  ```
- **설정 > 가계부에 참여하기**에서 초대코드 6자리 입력 후 “참여” 시 호출합니다.  
  이미 2명이면 `409 Conflict` 등으로 실패할 수 있으므로, 에러 메시지를 사용자에게 보여주세요.

### 4. GET `/api/v1/entries?ledgerId=123456`

- 쿼리 **필수**: `ledgerId` (6자리 숫자)
- 현재 선택된 가계부의 내역 목록을 가져올 때 사용합니다.

### 5. POST `/api/v1/entries`

- **Body 예시**
  ```json
  {
    "ledgerId": "123456",
    "type": "income",
    "amount": 10000,
    "title": "월급",
    "date": "2025-02-18",
    "memo": "선택"
  }
  ```
- `ledgerId`는 **필수**이며, 현재 사용자가 멤버인 가계부 ID여야 합니다.

### 6. PUT `/api/v1/entries/:id`

- **Body**: POST와 동일하게 `ledgerId` 포함 가능 (기존 내역의 가계부는 변경되지 않음).

### 7. POST `/api/v1/entries/import`

- **Body**
  ```json
  {
    "ledgerId": "123456",
    "entries": [
      { "type": "expense", "amount": 5000, "title": "점심", "date": "2025-02-18" }
    ]
  }
  ```
- 해당 가계부에 대해서만 일괄 import 합니다.

### 8. GET `/api/v1/summary?ledgerId=123456&period=2025-02`

- 쿼리 **필수**: `ledgerId`(6자리), `period`(YYYY-MM)  
- 선택된 가계부·선택된 월의 수입/지출/잔액 요약입니다.

### 9. PATCH `/api/v1/ledgers/:ledgerId` (가계부 설정 수정)

- **URL**: `ledgerId` = 설정할 가계부의 6자리 코드 (예: `PATCH /api/v1/ledgers/123456`)
- **Body**
  ```json
  { "name": "우리집 가계부" }
  ```
- **성공 시**
  ```json
  { "ledgerId": "123456", "name": "우리집 가계부" }
  ```
- **설정 > 가계부 설정**에서 가계부 번호(또는 목록에서 선택) 후 이름을 입력·저장할 때 호출합니다.
- 해당 가계부의 **멤버만** 수정할 수 있습니다. `name`은 100자 이하, 빈 문자열이면 이름 미설정(null)으로 저장됩니다.

### 10. DELETE `/api/v1/ledgers/:ledgerId`

- **URL**: `ledgerId` = 삭제할 가계부의 6자리 코드 (예: `DELETE /api/v1/ledgers/123456`)
- **Body**: 없음
- **성공 시**: `204 No Content` (응답 본문 없음)
- **설정 > 가계부 삭제**에서 삭제할 가계부 코드 6자리 입력 후 삭제 버튼 클릭 시 호출합니다.
- 해당 가계부의 **멤버만** 삭제할 수 있습니다. 멤버가 아니면 `404`.
- 삭제 시 해당 가계부의 **모든 내역**과 **멤버 소속**이 함께 삭제됩니다. 복구 불가이므로 확인/경고 UI 권장.

---

## 프론트엔드 작업 체크리스트

1. **설정 탭**
   - **가계부로 초대하기**: `POST /api/v1/ledgers/invite-code` 호출 후 응답의 `code`(또는 `ledgerId`)를 6자리로 표시. (복사하기 버튼 권장)
   - **가계부에 참여하기**: 6자리 입력 필드 + 참여 버튼 → `POST /api/v1/ledgers/join` with `{ "code": "123456" }`. 성공 시 `ledgerId`를 “내 가계부 목록”에 반영하고, 필요 시 해당 가계부로 전환.
   - **가계부 설정**: 가계부 번호 선택(드롭다운/목록) 후 “가계부 이름” 입력 필드 + 저장 버튼 → `PATCH /api/v1/ledgers/{ledgerId}` with `{ "name": "입력값" }`. 성공 시 목록 갱신(GET /api/v1/ledgers 재호출)하여 이름 반영.
   - **가계부 삭제**: 6자리 가계부 코드 입력 필드 + 삭제 버튼 → `DELETE /api/v1/ledgers/{코드}` 호출. 성공 시 목록에서 제거하고, 삭제한 가계부가 현재 선택 중이었으면 다른 가계부로 전환. 삭제 전 “정말 삭제하시겠어요? 내역이 모두 삭제됩니다.” 등 확인 다이얼로그 권장.

2. **가계부 선택**
   - 앱 로드 또는 설정 후 `GET /api/v1/ledgers`로 `ledgers` 배열 수신. 각 항목은 `{ ledgerId, name }`.
   - “현재 선택된 가계부” 상태를 하나 두고, 설정 또는 별도 UI로 전환 가능하게 처리. 표시 시 `name || ledgerId` 사용.
   - `ledgers`가 비어 있으면, “가계부로 초대하기”를 한 번 호출해 첫 가계부를 만들고 그 `ledgerId`를 선택 상태로 둡니다.

3. **내역/요약**
   - 모든 **entries** 호출에 현재 선택된 `ledgerId` 사용:  
     `GET /api/v1/entries?ledgerId=...`, POST/PUT/import body에 `ledgerId` 포함.
   - **summary** 호출 시 `ledgerId`와 `period` 모두 쿼리에 포함:  
     `GET /api/v1/summary?ledgerId=...&period=YYYY-MM`.

4. **에러 처리**
   - `409`: 가계부가 이미 2명일 때 참여 거부 → “이 가계부는 이미 2명이 사용 중이에요.” 등 안내.
   - `404`: 잘못된 초대코드 또는 해당 가계부 권한 없음 → 적절한 메시지 표시.

---

## 가계부 삭제 — 프론트 작업 시 복사용 프롬프트

아래 블록을 그대로 프론트엔드 에이전트에게 전달하면 됩니다.

```
설정 탭에 "가계부 삭제" 기능을 추가해줘.

- 삭제할 가계부의 6자리 코드를 입력받는 필드와 "삭제" 버튼을 둬.
- 삭제 시 API: DELETE /api/v1/ledgers/{ledgerId} (ledgerId에 입력한 6자리 코드)
- 인증: Authorization: Bearer <JWT>
- 성공 시(204): 내 가계부 목록 상태를 갱신하고(GET /api/v1/ledgers 다시 호출), 삭제한 가계부가 현재 선택 중이었으면 다른 가계부를 선택해줘.
- 삭제 전에 "이 가계부와 모든 내역이 삭제됩니다. 계속할까요?" 같은 확인 다이얼로그를 띄워줘.
- 에러(404 등) 시 사용자에게 메시지로 안내해줘.
```

---

## 가계부 설정(이름) — 프론트 작업 시 복사용 프롬프트

아래 블록을 그대로 프론트엔드 에이전트에게 전달하면 됩니다.

```
설정 탭에 "가계부 설정" 기능을 추가해줘.

- 내 가계부 목록(GET /api/v1/ledgers)으로 가계부를 선택할 수 있게 해. 응답은 { ledgers: [{ ledgerId, name }] } 형태야.
- 선택한 가계부의 "이름"을 입력할 수 있는 필드와 "저장" 버튼을 둬.
- 저장 시 API: PATCH /api/v1/ledgers/{선택한 ledgerId} , Body: { "name": "사용자 입력 이름" } (최대 100자)
- 인증: Authorization: Bearer <JWT>
- 성공 시 응답으로 { ledgerId, name } 이 오면, 목록 상태를 갱신하거나 선택한 가계부의 이름만 반영해줘.
- 가계부 선택은 드롭다운이나 리스트 중 하나로 하고, 표시할 땐 name이 있으면 name, 없으면 ledgerId(6자리 코드)를 보여줘.
- 에러(404, 400 등) 시 사용자에게 메시지로 안내해줘.
```

---

## 로컬 테스트

- 백엔드: `http://localhost:3001` (또는 `npm run start:dev` / Docker 포트)
- 위 API를 순서대로 호출해 보며,  
  “초대코드 생성 → 참여 → 내역/요약에 ledgerId 넣어 호출” 흐름이 동작하는지 확인하면 됩니다.
